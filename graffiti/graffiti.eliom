   ...
   (fun drawing ->
     match drawing with
       Line pts ->
         ...
         console (fun () ->
           "[draw_server] appending line to drawing_list (length of "
           ^ (string_of_int (List.length !drawing_list)) ^ ")");
     | Rectangle pts ->
       draw_rect ctx pts;
       drawing_list := (drawing :: !drawing_list);
       console (fun() ->
         "[draw_server] appending rectangle to drawing_list (length of "
         ^ (string_of_int (List.length !drawing_list)) ^ ")");
     | Circle pts ->
       draw_circle ctx pts;
       drawing_list := (drawing :: !drawing_list);
       console (fun () ->
         "[draw_sever] appending circle to drawing_list (length of "
         ^ (string_of_int (List.length !drawing_list)) ^ ")");
   ),
   (fun() ->
     let surface = Cairo.Image.create Cairo.Image.ARGB32 ~width  ~height in
...
let imageservice =
  ...
  {client{
    let init_client () =
      Eliom_lib.alert
        "[init_client] drawing_list of length %s"
        (string_of_int(List.length !(%drawing_list)));
      ...
      let press key =
        let () = Eliom_lib.debug "[press] charCode: %i" key in
        ...
        (fun () ->
          ...
          mousedowns canvas
            (fun ev _ ->
              ...
              Lwt.pick [mousemoves Dom_html.document
                           (fun x _ ->
                             let _ = Lwt.bind
                               (%rpc_get_key ())
                               (fun k -> key := !k; Lwt.return()) in
                             if !key = 108 then line x else Lwt.return());
